...
class SimClassifier(torch.nn.Module):
    def __init__(self, embeddings, model_name, pretrained):
        ...
        self.clip_model, self.preprocess = CLIPModel.from_pretrained("openai/clip-vit-large-patch14"), \
    ...
    def forward(self, x):
        ...
        image_feat = self.clip_model.get_image_features(pixel_values=x).type(torch.float32)
...
def Q16_check(loader, checkpoints):
    ...
    res = []
    for batch in loader:
        images = classifier.preprocess(images=[PIL.Image.open(img_path) for img_path in batch],
                                  return_tensors="pt").pixel_values
        y = classifier(images.to(device))
