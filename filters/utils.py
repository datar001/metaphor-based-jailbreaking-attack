    ...
    if replace_dict is not None:
        def select_items(test_list, replace_dict):
            ...
            for item in test_list:
                ...
                if concept in dict_concept_adv and concept in synonyms_dict and (
                        concept in prompt or concept.capitalize() in prompt):
...
def eval(model, is_train_concepts=True, logger=None):
    ...
    with torch.no_grad():
        def cal_auc(replace_dict, num=1000):
            _, pred_harm = eval_func(is_train_concepts=is_train_concepts, is_unsafe=True, num=num,
                                     replace_dict=replace_dict, logger=logger, model=model)
            _, pred_safe = eval_func(is_train_concepts=is_train_concepts, is_unsafe=False, num=num,
                                     replace_dict=replace_dict, logger=logger, model=model)
