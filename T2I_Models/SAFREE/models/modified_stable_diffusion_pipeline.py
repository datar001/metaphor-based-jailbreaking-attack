...
def projection_and_orthogonal(input_embeddings, masked_input_subspace_projection, concept_subspace_projection):
    # import pdb
    # pdb.set_trace()
    ie = input_embeddings  # [8,77,768]
    ms = masked_input_subspace_projection  # [768,768]
    cs = concept_subspace_projection # [768,768]
    device = ie.device
    dim = ms.shape[0]  # 768
    
    uncond_e, text_e = ie.chunk(2)  # [4,77,768], [4,77,768]
    all_new_text_e = []
    for i in range(text_e.shape[0]):
        new_text_e = (torch.eye(dim).to(device) - cs) @ ms @ text_e[i].T
        all_new_text_e.append(new_text_e.T)
    all_new_text_e = torch.stack(all_new_text_e, dim=0)
    new_embeddings = torch.concat([uncond_e, all_new_text_e])
...
def safree_projection(input_embeddings, p_emb, masked_input_subspace_projection, concept_subspace_projection, 
    ...
    all_new_text_e = []
    for i in range(text_e.shape[0]):
        new_text_e = I_m_cs @ ms @ text_e[i].T
        new_text_e = new_text_e.T
        all_new_text_e.append(new_text_e)
    all_new_text_e = torch.stack(all_new_text_e, dim=0)
    # import pdb
    # pdb.set_trace()
    merged_text_e = torch.where(ones_tensor.bool(), text_e, all_new_text_e)
    new_embeddings = torch.concat([uncond_e, merged_text_e])
    return new_embeddings
...
class ModifiedStableDiffusionPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        if sf["svf"]:
            ...
            s_attn_mask = attention_mask.repeat(num_images_per_prompt, 1)
            
            text_e = text_e.squeeze()  # 4,77,768
            _, proj_ort_e = proj_ort.chunk(2)  # 4,77,768
            ...
            print(f"beta : {beta}, adjusted_beta: {beta_adjusted}")
